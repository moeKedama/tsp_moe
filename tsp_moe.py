...
class GA(object):
    ...
    def compute_dis_mat(self, num_city, location):
        ...
        for i in range(num_city):
            for j in range(num_city):
                if i == j:
                    dis_mat[i][j] = np.inf     # 距离自己的距离为无穷大
                    continue
                a = location[i]
                b = location[j]
    ...
    def swap_part(self, list1, list2):          # 没什么用的交换模块
        index = len(list1)
        list = list1 + list2
        list = list[::-1]
    ...
    def ga_choose(self, genes_score, genes_choose):    # 轮盘赌选择
        sum_score = sum(genes_score)
        score_ratio = [sub * 1.0 / sum_score for sub in genes_score]
        rand1 = np.random.rand()
        rand2 = np.random.rand()
        for i, sub in enumerate(score_ratio):        # genes_score顺序可能对轮盘赌有影响，是否要shuffle？
            if rand1 >= 0:
                rand1 -= sub
                if rand1 < 0:
                    index1 = i
    ...
    def ga_mutate(self, gene):          # 逆转
        path_list = [t for t in range(len(gene))]
        order = list(random.sample(path_list, 2))
        start, end = min(order), max(order)
        tmp = gene[start:end]
        # np.random.shuffle(tmp)   # 全部打乱(显然不行)
        tmp = tmp[::-1]
        gene[start:end] = tmp

        if np.random.rand() < self.variation_radio:               # 逆转时有几率点对点变异
            order = list(random.sample(path_list, 2))
            start, end = min(order), max(order)
            tmp = gene[start]
            gene[start] = gene[end]
    ...
    def run(self):
        ...
        for i in range(1, self.iteration + 1):
            ...
            if tmp_best_score > best_score:
                ...
                self.best_epoch = i
                print("The recent best routine is")
                print(tmp_best_one)
            print("Epoch:" + str(i) + " ,the score is " + str(1. / tmp_best_score))
        print("THE EPOCH " + str(self.best_epoch) + " REACHES THE BEST RESULT")
        print("The best score is " + str(1./best_score))
        print("The best routine is")
        print(BEST_LIST)
        # plt.subplot(2, 2, 4)
...
plt.scatter(data[:, 0], data[:, 1], marker=',', s=10)
# plt.plot(data[:, 0], data[:, 1], linewidth=1.0)
plt.show()
...
# plt.subplot(2, 2, 3)
# 加上一行因为会回到起点
Best_path = np.vstack([Best_path, Best_path[0]])    # 存放点坐标的顺序
plt.plot(Best_path[:, 0], Best_path[:, 1], linewidth=1.0)
plt.title('result')
plt.show()
